...
// use the useEffect hook to call api on page render (GET api/billing-manager/users?status=invited&role_id=1&p=1 )
...
const InvitedUsers = () => {
  ...
  useEffect(() => {
    async function getData() {
      ...
      if (res.status === 200) {
        const invitedUserData = formateUserData(ans.data.rows);
        setUsers(invitedUserData);
  ...
  const handleInvitedCTA = async (email) => {
    try {
